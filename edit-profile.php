<?php
   ...
   if(!isset($_SESSION['ID'])){
    ...
    if(isset($_POST['save'])){
        ...
        if(empty($error)){
            $stmt = $connect->prepare("UPDATE users SET email = ?, first_name = ?, last_name = ?, password = ?, image = ?  WHERE id = ?");
            $result = $stmt ->execute([$email, $first_name, $last_name, $new_pass, $image,  $user['id']]);

            if($result == true) {
                $_SESSION['message'] = "تحديث المستخدم بنجاح";
            }
...
<?php if(isset($_SESSION['message'])): ?>
    <div class="alert alert-success" style="width:85%;margin:0 auto;">
        <?= $_SESSION['message'] ?>
    </div>
